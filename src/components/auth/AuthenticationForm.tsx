...
export function AuthenticationForm() {
    ...
    validate: {
      ...
      password: (value) => {
        if (type === "signUp") {
          ...
          if (!/[$&+,:;=?@#|'<>.^*()%!-]/.test(value)) return "Password must include a special symbol";
        }
      ...
      confirmPassword: (value, values) => (type === "signUp" && value !== values.password ? "Passwords do not match" : null),
      code: (value) =>
        (type === "verify" || type === "reset") && value.length !== 6 ? "Verification code must be 6 digits" : null,
      newPassword: (value) => {
        if (type === "reset") {
          ...
          if (!/[$&+,:;=?@#|'<>.^*()%!-]/.test(value)) return "New password must include a special symbol";
        }
        return null;
      },
    },
    ...
    try {
      ...
      const errorMessage =
        error instanceof ConvexError
          ? (error.data as { message: string }).message || "Failed to login with GitHub"
    ...
    async (values: AuthFormValues) => {
      ...
      try {
        if (type === "signIn" || type === "signUp" || type === "forgot") {
          const userExists = await convex.query(api.users.checkUserExists, { email: normalizedEmail });
          ...
          if (type === "signIn" && userExists) {
            const providers = await convex.query(api.users.checkUserProvider, { email: normalizedEmail });
            if (providers?.includes("github") && !providers.includes("password")) {
              setIsLoading(false);
              setError("You are already registered with GitHub. Please use GitHub to log in.");
              return;
            }
          }
        ...
        if (type === "signIn" && !result.signingIn) {
          const isVerified = await convex.query(api.users.getUserVerificationStatus, { email: normalizedEmail });
          if (!isVerified) {
          ...
          try {
            ...
            const errorMessage =
              verifyError instanceof ConvexError
                ? (verifyError.data as { message: string }).message || "Failed to verify email"
  ...
  return (
    <Center h="100dvh">
      <Paper radius="md" p="xl" withBorder className="w-full max-w-md" aria-labelledby="auth-title">
        ...
        {(type === "signIn" || type === "signUp") && (
          <>
            ...
            <Divider label="Or continue with email" labelPosition="center" mt="xl" mb="lg" />
          </>
        )}
          ...
          <fieldset disabled={isLoading}>
            {(type === "signIn" || type === "signUp" || type === "forgot") && (
              <>
                ...
                <TextInput placeholder="your@email.com" radius="md" aria-label="Email address" {...form.getInputProps("email")} />
              </>
            )}
            ...
            {type === "signIn" && (
              <>
                ...
                <PasswordInput placeholder="••••••••" radius="md" aria-label="Password" {...form.getInputProps("password")} />
              </>
            )}
            ...
            <Group justify="space-between" mt="xl">
              <Anchor
                ...
                onClick={() =>
                  toggleType(
                    type === "signIn"
                      ? "signUp"
                      : type === "forgot" || type === "verify" || type === "reset"
                        ? "signIn"
                        : "signIn",
                  )
                }
              ...
              >
                {type === "signIn" ? "Login" : type === "signUp" ? "Register" : type === "forgot" ? "Forgot" : "Continue"}
              </Button>
            </Group>
          </fieldset>
        </form>
